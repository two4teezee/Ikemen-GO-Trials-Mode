#!/usr/bin/env python3
"""Generate external/mods/trials/trials.sff and trials.air.
...
# group, number, canvas, and the name the .air and system.def know it by. The numbering
# is grouped by role rather than run consecutively, so a screenpack reading the .air can
# tell a block background from a Step background without a legend.
#
...
AIR_HEADER = """; TRIALS MODE — the module's own default artwork, as actions
;
; Generated by tools/make-trials-sff.py. Edit that, not this.
;
; Every action here is one frame of one sprite in trials.sff, held indefinitely. They
; exist so that a Trials Config key can name artwork as `anim = N` rather than as
; `spr = group, index` — see the COMMAND GLYPHS and STEP BACKGROUNDS blocks in
; system.def. The module's shipped defaults use `spr`, because that route needs no
; action lookup at all; `anim` is here for a screenpack that wants to animate the
; module's own sprites without shipping an sff of its own.
;
; A screenpack's own `anim = N` means an action in ITS air, not one below. Which file a
; number is resolved against follows whoever wrote the key, exactly as the coordinate
; space does.
"""
...
def write_air(path):
    lines = [AIR_HEADER]
    for i, (group, number, canvas, label) in enumerate(SPRITES):
        lines.append(
            "; %s — %dx%d\n[Begin Action %d]\n%d,%d, 0,0, -1\n"
            % (label, canvas.w, canvas.h, i, group, number)
        )
    text = "\n".join(lines)
...
def main():
    ...
    air = os.path.normpath(os.path.join(MOD, "trials.air"))
    n = write_sff(sff)
    m = write_air(air)
    print("wrote %s (%d bytes, %d sprites)" % (sff, n, len(SPRITES)))
    print("wrote %s (%d bytes)" % (air, m))
    return 0
