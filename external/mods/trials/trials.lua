...
-- The module's own artwork, and the actions naming it: external/mods/trials/trials.sff
-- and trials.air, generated by tools/make-trials-sff.py.
--
...
-- the largest sprites either file holds.
--
...
-- trials.air, as a table of action number to the text of that action.
--
-- Read as text and split here rather than through the engine's own reader, because the
-- engine has no Lua entry point for one: ReadAnimationTable runs in Go and the table it
-- fills reaches Lua empty (see buildAnim). animNew does take the text of an action and
-- splits it on newlines itself (src/anim.go:2042), so an action lifted out of the file
-- verbatim is read exactly as the engine would have read it.
local moduleActionsRead, moduleActionsData = false, nil
local function moduleActions()
	if moduleActionsRead then
		return moduleActionsData
	end
	moduleActionsRead = true
	moduleActionsData = {}
	local ok, text = pcall(loadText, trials.dir .. 'trials.air')
	if not ok or type(text) ~= 'string' then
		return moduleActionsData
	end
	local lines, current = nil, nil
	for line in (text .. '\n'):gmatch('([^\n]*)\n') do
		local no = line:match('^%s*%[%s*[Bb]egin%s+[Aa]ction%s+(%-?%d+)%s*%]')
		if no ~= nil then
			if lines ~= nil then
				moduleActionsData[current] = table.concat(lines, '\n')
			end
			current, lines = tonumber(no), {}
...
--
-- `actions` is a table `anim = N` is resolved against, and is the module's own when the
-- key naming the artwork came from the module's own layer. A screenpack's `anim = N`
-- has no such table and falls through to the engine's, which will not fire on a stock
-- build: `AnimationTable`'s only fields are unexported, so the reflection in toLValue
-- hands Lua an empty `motif.AnimTable` and a screenpack's [Begin Action N] is
-- unreachable from a pure-Lua module. `spr = group, index` against an Sff is the route
-- that works for a screenpack, and is what the module's own defaults use too. The
-- engine branch stays so that a build which does expose the table gets the engine's own
-- priority order for free.
local function buildAnim(path, sff, origin, actions)
	local g = elementReader(path, origin)
	local geo = elementGeometry(g, path, origin)
	...
	local a = nil
	if anim ~= nil and anim >= 0 and type(actions) == 'table' and actions[anim] ~= nil then
		a = animNew(sff, actions[anim])
	elseif anim ~= nil and anim >= 0 and type(motif.AnimTable) == 'table' and motif.AnimTable[anim] ~= nil then
...
local function buildArt(path, origin, foreign)
	if artIsOurs(path, origin) then
		...
		return buildAnim(path, sff, origin, moduleActions())
	end
